...
export async function getOKH(
  ...
  try {
    ...
    // const cloned_products = JSON.parse(JSON.stringify(example_products)) as typeof example_products;
...
export async function listOKHsummaries(
  ...
  const promises = data.map((item) => {
    ...
    return fname != "okh-seat-helpful"
      ? getOKHByFileName(fname, "okh", extension)
      : Promise.resolve(undefined);
  });
  const values = (await Promise.all(promises)).filter((item) => item !== undefined);
