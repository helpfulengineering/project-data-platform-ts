...
- **`GET /getRelatedOKH` ignores the `keywords` query param entirely.** The route has no `{keywords}` path template, so `request.params.keywords` is `undefined`; `decodeURIComponent(undefined)` coerces to the literal string `"undefined"`, so the effective filter is always `["undefined"]` no matter what's queried. Captured as a regression baseline in `packages/back-end/test/api.test.ts` (asserts two different queries return identical results, rather than hardcoding a specific blob-dependent response) — fixing the actual keyword-matching bug is tracked in `CLEANUP_PLAN.md`, not done.
